...
class UserFriendshipsControllerTest < ActionController::TestCase
	...
	context "#index" do 
		...
		context "when logged in" do
			setup do
				@friendship1 = create(:pending_user_friendship, user: users(:david), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
				@friendship2 = create(:accepted_user_friendship, user: users(:david), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
				sign_in users(:david)
				get :index
			end
			...
			should "display pending information on accepted friendship" do
	...
	context "#new" do 
		...
		context "when logged in" do
			...
			should "get new nad return success" do
				get :new
				assert_response :success
			end
			...
			should "display the friend's name" do
				get :new, friend_id: users(:gil)
				assert_match /#{users(:gil).full_name}/, response.body
			end
			...
			should "assign a new user friendship to the correct friend" do
				get :new, friend_id: users(:gil)
				assert_equal users(:gil), assigns(:user_friendship).friend 
			end

			should "assign a new user friendship to the currently logged in user" do
				get :new, friend_id: users(:gil)
				assert_equal users(:david), assigns(:user_friendship).user
			end
			...
			should "ask if you really want to friend the user" do
				get :new, friend_id: users(:gil)
				assert_match /Do you really want to friend #{users(:gil).full_name}?/, response.body
			end
		end
	end
	...
	context "#accept" do
		...
		context "when logged in" do
			setup do
				@user_friendship = create(:pending_user_friendship, user: users(:david))
				sign_in users(:david)
			...
			should "update the state to accept" do 
					assert_equal 'accepted', @user_friendship.state
			end

			should "ahve a flash success message" do 
